...
public class UserLinks : IUserLinks
{
    ...
    public UserLinks(LinkGenerator linkGenerator, IDataShaper<UserForDisplayDto, string> dataShaper)
    {
    ...
    public LinkResponse TryGenerateLinks(IEnumerable<UserForDisplayDto> entitiesDto, string fields, HttpContext httpContext)
    {
    ...
    private List<Entity> ShapeData(IEnumerable<UserForDisplayDto> entitiesDto, string fields) =>
    ...
    private LinkResponse ReturnLinkdedEntities
        (IEnumerable<UserForDisplayDto> entitiesDto, string fields, HttpContext httpContext, List<Entity> shapedEntities)
    {
    ...
    private List<Link> CreateLinksForEntity(HttpContext httpContext, string id, string fields = "")
    {
        ...
        {
            new
            Link(_linkGenerator.GetUriByAction(httpContext,"GetUser", values: new { id, fields })!, "self","GET"),
        };
    ...
    private LinkCollectionWrapper<Entity> CreateLinksForEntities(HttpContext httpContext, LinkCollectionWrapper<Entity> entitiesWrapper)
    {
        entitiesWrapper.Links.Add(new Link
            (_linkGenerator.GetUriByAction(httpContext, "GetUsers", values: new { })!, "self", "GET"));
